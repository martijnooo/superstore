...
```{r}
ggplot(superstore, aes(y = Profit, x = Ship.Mode)) +
  geom_point(m) +
  scale_x_discrete(limits = c("Standard Class", "First Class", "Second Class", "Same Day"))
...
```{r}
# Linear regression to model Profit based on Ship Mode
model <- lm(Profit ~ Ship.Mode, data = superstore)
